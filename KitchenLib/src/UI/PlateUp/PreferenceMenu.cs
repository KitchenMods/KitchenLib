...
namespace KitchenLib.UI
{
	internal class PreferenceMenu<T> : KLMenu<T>
	{
		...
		{
			{ (Main.cosmeticManager.GetPreference<PreferenceBool>("isPlateUpDeveloper").Value, GDOUtils.GetCustomGameDataObject<ItsHappeningCape>().ID), "Its Happening! Cape" },
			{ (Main.cosmeticManager.GetPreference<PreferenceBool>("isPlateUpSupport").Value, GDOUtils.GetCustomGameDataObject<PlateUpSupportCape>().ID), "Support Cape" },
			{ (Main.cosmeticManager.GetPreference<PreferenceBool>("isPlateUpStaff").Value, GDOUtils.GetCustomGameDataObject<PlateUpStaffCape>().ID), "Staff Cape" },
			{ (Main.cosmeticManager.GetPreference<PreferenceBool>("isKitchenLibDeveloper").Value, GDOUtils.GetCustomGameDataObject<KitchenLibCape>().ID), "KitchenLib Cape" },
			{ (Main.cosmeticManager.GetPreference<PreferenceBool>("isTwitchStreamer").Value, GDOUtils.GetCustomGameDataObject<TwitchCape>().ID), "Twitch Cape" },
			{ (Main.cosmeticManager.GetPreference<PreferenceBool>("isEasterChampion").Value, GDOUtils.GetCustomGameDataObject<EasterCape>().ID), "Easter Champion Cape" },
			{ (Main.cosmeticManager.GetPreference<PreferenceBool>("isGearsChampion").Value, GDOUtils.GetCustomGameDataObject<GearsCape>().ID), "Gears Champion Cape" },
		};
